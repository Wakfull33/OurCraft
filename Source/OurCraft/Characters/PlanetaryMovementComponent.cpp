...
FVector UPlanetaryMovementComponent::GetGravityDirection(bool bAvoidZeroGravity) const
{
...
float UPlanetaryMovementComponent::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
...
void UPlanetaryMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	if (!HasValidData())
	{
		return;
...
void UPlanetaryMovementComponent::PerformMovement(float DeltaSeconds)
{
	...
	// Scoped updates can improve performance of multiple MoveComponent calls.
	{
		...
		if (HasAnimRootMotion())
		{
			...
			if (DeltaSeconds > 0.f)
			{
				CalcAnimRootMotionVelocity(RootMotionParams.GetRootMotionTransform().GetTranslation(), DeltaSeconds, Velocity);
			}
		}
...
void UPlanetaryMovementComponent::HandleImpact(const FHitResult& Hit, float TimeSlice /*= 0.f*/, const FVector& MoveDelta /*= FVector::ZeroVector*/)
{
	if (CharacterOwner)
	{
...
void UPlanetaryMovementComponent::ProcessLanded(const FHitResult& Hit, float remainingTime, int32 Iterations)
{
...
void UPlanetaryMovementComponent::MaintainHorizontalGroundVelocity()
{
	return;
...
float UPlanetaryMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& Normal, FHitResult& Hit, bool bHandleImpact)
{
	if (!Hit.bBlockingHit)
	{
		return 0.0f;
...
void UPlanetaryMovementComponent::SetPostLandedPhysics(const FHitResult& Hit)
{
	if (CharacterOwner)
	{
...
void UPlanetaryMovementComponent::PhysWalking(float deltaTime, int32 Iterations)
{
...
FVector UPlanetaryMovementComponent::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace) const	// OK
{
...
void UPlanetaryMovementComponent::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FStepDownResult* OutStepDownResult /*= NULL*/)
{
	if (!CurrentFloor.IsWalkableFloor())
	{
		return;
...
void UPlanetaryMovementComponent::SimulateMovement(float DeltaTime)
{
...
FVector UPlanetaryMovementComponent::ConstrainInputAcceleration(const FVector& InputAcceleration) const
{
...
FVector UPlanetaryMovementComponent::ScaleInputAcceleration(const FVector& InputAcceleration) const
{
	return GetMaxAcceleration() * GetClampedToMaxSizePrecise(InputAcceleration, 1.0f);
}

FVector UPlanetaryMovementComponent::CalcAnimRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaTime, const FVector& CurrentVelocity) const
{
...
void UPlanetaryMovementComponent::StartFalling(int32 Iterations, float remainingTime, float timeTick, const FVector& Delta, const FVector& subLoc)
{
	const float DesiredDist = Delta.Size();
...
void UPlanetaryMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
FVector UPlanetaryMovementComponent::GetFallingLateralAcceleration(float DeltaTime)
{
	const FVector GravityDirection = GetGravityDirection();
...
FVector UPlanetaryMovementComponent::NewFallVelocity(const FVector& InitialVelocity, const FVector& Gravity, float DeltaTime) const
{
...
void UPlanetaryMovementComponent::UpdateBasedMovement(float DeltaSeconds)
{
	if (!HasValidData())
	{
		return;
...
FVector UPlanetaryMovementComponent::GetImpartedMovementBaseVelocity() const
{
...
void UPlanetaryMovementComponent::JumpOff(AActor* MovementBaseActor)
{
...
void UPlanetaryMovementComponent::FindFloor(const FVector& CapsuleLocation, FFindFloorResult& OutFloorResult, bool bZeroDelta, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
	...
	{
		UPlanetaryMovementComponent* MutableThis = const_cast<UPlanetaryMovementComponent*>(this);
...
void UPlanetaryMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
...
// void UPlanetaryMovementComponent::SetUpdatedComponent(USceneComponent* NewUpdatedComponent)
// {
...
// 	{
// 		OldPrimitive->OnComponentBeginOverlap.RemoveDynamic(this, &UPlanetaryMovementComponent::CapsuleTouched);
// 	}
// 
...
// 	{
// 		UpdatedPrimitive->OnComponentBeginOverlap.AddUniqueDynamic(this, &UPlanetaryMovementComponent::CapsuleTouched);
// 	}
// 
// 	if (bUseRVOAvoidance)
// 	{
...
bool UPlanetaryMovementComponent::FloorSweepTest(struct FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel,
...
bool UPlanetaryMovementComponent::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
bool UPlanetaryMovementComponent::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetCapsuleAxisZ();
...
bool UPlanetaryMovementComponent::ShouldComputePerchResult(const FHitResult& InHit, bool bCheckRadius /*= true*/) const
{
...
bool UPlanetaryMovementComponent::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FFindFloorResult& OutPerchFloorResult) const
{
...
bool UPlanetaryMovementComponent::CanStepUp(const FHitResult& Hit) const
{
...
bool UPlanetaryMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult &Hit, struct UCharacterMovementComponent::FStepDownResult* OutStepDownResult /*= NULL*/)
{
...
void UPlanetaryMovementComponent::SetMovementMode(EMovementMode NewMovementMode, uint8 NewCustomMode /*= 0*/)
{
...
void UPlanetaryMovementComponent::SetDefaultMovementMode()
{
...
void UPlanetaryMovementComponent::AdjustFloorHeight()
{
...
bool UPlanetaryMovementComponent::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir) const
{
...
FVector UPlanetaryMovementComponent::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir) const
{
...
void UPlanetaryMovementComponent::StartNewPhysics(float deltaTime, int32 Iterations)
{
...
void UPlanetaryMovementComponent::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
...
void UPlanetaryMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
...
bool UPlanetaryMovementComponent::IsWalkable(const FHitResult& Hit) const
{
	if (!Hit.IsValidBlockingHit())
	{
...
void UPlanetaryMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult /*= NULL*/) const
{
	OutFloorResult.Clear();
...
bool UPlanetaryMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const FVector CapsuleDown = GetCapsuleAxisZ() * -1.0f;
...
FVector UPlanetaryMovementComponent::GetGravity() const
{
	if (!CustomGravityDirection.IsZero())
	{
...
FVector UPlanetaryMovementComponent::GetComponentDesiredAxisZ() const
{
	return GetGravityDirection(true) * -1.0f;
}

void UPlanetaryMovementComponent::UpdateComponentRotation()
{
	if (!UpdatedComponent)
	{
...
FORCEINLINE FQuat UPlanetaryMovementComponent::GetCapsuleRotation() const
{
	return UpdatedComponent->GetComponentQuat();
}

FORCEINLINE FVector UPlanetaryMovementComponent::GetCapsuleAxisX() const
{
...
FORCEINLINE FVector UPlanetaryMovementComponent::GetCapsuleAxisZ() const
{
...
// Version that does not use inverse sqrt estimate, for higher precision.
FORCEINLINE FVector UPlanetaryMovementComponent::GetSafeNormalPrecise(const FVector& V)
{
	const float VSq = V.SizeSquared();
	if (VSq < SMALL_NUMBER)
	{
...
bool UPlanetaryMovementComponent::IsWithinEdgeToleranceNew(const FVector& CapsuleLocation, const FVector& CapsuleDown, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	const float DistFromCenterSq = (CapsuleLocation + CapsuleDown * ((TestImpactPoint - CapsuleLocation) | CapsuleDown) - TestImpactPoint).SizeSquared();
	const float ReducedRadiusSq = FMath::Square(FMath::Max(KINDA_SMALL_NUMBER, CapsuleRadius - SWEEP_EDGE_REJECT_DISTANCE));
